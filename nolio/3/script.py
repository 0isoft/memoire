...
folder = "/home/octavian/projects/memoire/nolio/3"
...
def extract_tcx_datetime(root):
    for elem in root.iter():
        if strip_ns(elem.tag) == "Id":
...
def parse_tcx(file_path):
    ...
    start_time = extract_tcx_datetime(root)

    return start_time,total_time/60, moving_time/60, total_distance/1000

# --- FIT PARSER ---


def extract_fit_datetime(fitfile):
    for msg in fitfile.get_messages("session"):
        for data in msg:
            if data.name == "start_time":
                return data.value
    return None
def extract_fit_datetime_from_records(fitfile):
...
def parse_fit(file_path):
    ...
    start_time = extract_fit_datetime(fitfile)
    if start_time is None:
        start_time = extract_fit_datetime_from_records(fitfile)
...
for file in os.listdir(folder):
    ...
    try:
        if file.lower().endswith(".tcx"):
            start_time,elapsed_min, moving_min, dist_km = parse_tcx(path)


        elif file.lower().endswith(".fit"):
            start_time,elapsed_min, moving_min, dist_km = parse_fit(path)
